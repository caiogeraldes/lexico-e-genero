...
def gera_paragrafo(data: pd.DataFrame) -> pd.DataFrame:
    ...
    j: Any = data.groupby(['author', 'text', 'file','div_sup', 'par', 'genero']).agg({'lemma': lambda x: " ".join(x)})
...
def gera_sent(data: pd.DataFrame) -> pd.DataFrame:
    ...
    data.dropna(inplace=True)
    j: Any = data.groupby(['author', 'text', 'file', 'sent_id', 'genero']).agg({'lemma': lambda x: " ".join(x)})
